...
3. Добавляем событие `turn_complete` с полными текстами:
   - `user_text`
   - `bot_text`
4. Дашборд MVP:
   - Обзор
...
- `turn_number`
- `user_text` (полный текст, с лимитом длины)
- `bot_text` (полный итоговый текст, с лимитом длины)
- `intent`
- `doc_id` (или null)
...
## 11) Известный баг (обязательный фикс перед cost-аналитикой)
...
Статус: must-fix до запуска экрана "Стоимость".
