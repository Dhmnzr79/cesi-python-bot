...
## 5. Policy layer — таблица правил

```python
# Порядок проверки: первое сработавшее правило побеждает

1. leadIntent != none          → воронка, остальное не трогать
2. turn_count < 2              → CTA не показывать (кроме явного намерения)
3. пользователь углубляется    → следующая подтема из конфига
4. финальная подтема           → мостик в соседнюю тему или CTA
5. turn_count >= 4, нет CTA    → предложить «рассказать о ситуации»
6. сменилась тема              → сбросить глубину старой темы
7. вопрос про цену             → мостик в рассрочку / что входит
8. ситуация уже показана       → повторно не показывать (per topic)
9. handoff = true              → только передача, без кнопок
10. default                    → next_buttons из YAML конфига
```
